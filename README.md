Para para clonar el repositorio

1.Iniciar un repositorio de visual studio code

2.Realizar el comando git .init para iniciar el repositorio

3. Utilizar el comando git clone junto a la URL mediante HTTPS: https://github.com/ElxCoNe/prueba-html-riwi.git

4.Ingresar a la carpeta que se crea automaticamente

5. Encontrarás todos los recursos utilizados para llevar a cabo este proyecto



**INFORMACION CODER

Nombre: Juan Camilo Hernández Ortega
Clan: Clan McCarthy
Correo: jchernandez10ocm@gmail.com
Documento de identidad: 1017270052


**Recursos utilizados
Todas la imagenes,iconos y fuentes fueron extraidas del figma: 
https://www.figma.com/design/hw5hfbrYc2SDWhcBjZeVqV/Responsive-Landing-Page-Design-%7C-Website-Home-Page-Design-%7C-Agency-Website-UI-Design--Community-?node-id=0-1&p=f&t=cGtDkgo8FHt4Z8E6-0

Igualmente se encuentran la carpeta img de mi proyecto

